        ...
        <form>
          <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
            ...
            <div class="">
              ...
              <input type="text" class="form-input" value="{{ $user->uid }}" id="uid" disabled />
            </div>
            <div class="">
          ...
          <div class="mt-5 grid grid-cols-1 gap-5 md:mt-8 md:grid-cols-2">
            ...
            <div class="">
              <label for="password" class="mb-2 block font-hk text-secondary">Password 확인</label>
              <input
                type="password"
                class="form-input"
                value=""
          ...
          <div class="my-8 border-t-2 pt-6">
            ...
            @php
              $address = \App\Models\UserAddress::where('user_id', $user->id)->where('type', 'main')->first();
            @endphp
            <div class="mt-5 grid grid-cols-1 gap-5 md:grid-cols-3">
              <div class="">
                <label for="zip" class="mb-2 block font-hk text-secondary">우편번호</label>
                <input type="number" class="form-input" name="zipcode" value="{{ $address->zipcode ?? '' }}" />
              </div>
            </div>
            <div class="mt-5 grid grid-cols-1 gap-5 md:grid-cols-2">
              <div class="w-full">
                <label for="street" class="mb-2 block font-hk text-secondary">주소1</label>
                <input type="text" class="form-input" id="address1" name="address1" onClick="execDaumPostCode()" value="{{ $address->address1 ?? '' }}" />
              </div>
              <div class="w-full">
                <label for="street2" class="mb-2 block font-hk text-secondary">주소2</label>
                <input type="text" class="form-input" name="address2" value="{{ $address->address2 ?? '' }}" />
              </div>
            </div>
            <div class="mt-5 grid grid-cols-1 gap-5 md:grid-cols-3">
               <div class="">
                <label for="phone" class="mb-2 block font-hk text-secondary">핸드폰 번호</label>
                <input type="text" class="form-input" name="phone" value="{{ $address->phone ?? '' }}" />
              </div>
            </div>
          </div>

          <div class="grid justify-end">
            <button type="button" class="btn btn-primary" aria-label="Save button" onclick="edit();">
              수정하기
            </button>
          </div>
        </form>
...
function edit() {
  ...
  if (!isFormValid) {
    ...
    for (let pair of formData.entries()) {
      ...
      if (pair[0] == 'password1' && pair[1] !== "") {
        $p1 = pair[1];
      }
      if (pair[0] == 'password2' && pair[1] !== "") {
        $p2 = pair[1];
      }
    }
    if ($p1 !== $p2) {
    ...
    const url = `{{ route('account.update') }}`;
